...
class Flink118TimestampTest
  ...
  protected val sinkForLongsResultsHolder: () => TestResultsHolder[lang.Long] =
...
class FixedWatermarks(customFixedTime: Long) extends AssignerWithPunctuatedWatermarks[String]() {
  ...
  override def checkAndGetNextWatermark(lastElement: String, extractedTimestamp: Long): Watermark = new Watermark(
    extractedTimestamp
  )
...
class TestCreator(
    assigner: Option[TimestampWatermarkHandler[String]],
...
object CheckTimeTransformer extends CustomStreamTransformer {
  ...
  @MethodToInvoke
  def invoke(): FlinkCustomStreamTransformation = { (start: DataStream[Context], _: FlinkCustomNodeContext) =>
    {
      start
        .keyBy((_: Context) => "")
        .process(new KeyedProcessFunction[String, Context, ValueWithContext[AnyRef]] {

          override def processElement(
              value: api.Context,
              ctx: KeyedProcessFunction[String, api.Context, ValueWithContext[AnyRef]]#Context,
          ...
          override def onTimer(
              timestamp: Long,
              ctx: KeyedProcessFunction[String, api.Context, ValueWithContext[AnyRef]]#OnTimerContext,
              out: Collector[ValueWithContext[AnyRef]]
          ): Unit = {
